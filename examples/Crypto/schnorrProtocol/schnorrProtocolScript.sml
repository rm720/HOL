...
open groupTheory; (* FiniteAbelianGroup_def *)
open arithmeticTheory; (* NOT_ZERO *)
open dividesTheory; (* NOT_PRIME_0*)
open fieldTheory;
...
val _ = new_theory "schnorrProtocol";
...
Proof
  ...
  rpt strip_tac >-
   ...
   Cases_on ‘e1 = e2’ THENL [
       ...
       ‘(GF q).sum.inv e2 = (GF q).sum.inv e1’ by rw[]>>
       ‘de = (e1 ⊖ e2)’ by rw[] >>
       ‘de = _0_’by rw[field_sub_eq_zero]>>
       ‘de ∈ (GF q).carrier’ by rw[GF_field]>>
       ‘Inv (GF q) de  ∈ (GF q).carrier’  by rw[field_inv_element]>>
           
       ‘_0_ ∈ (GF q).carrier’ by rw[GF_field]>>
       ‘Inv ((GF q) (e1 ⊕ (GF q).sum.inv e2)) ) ∈ (GF q).carrier’ by rw[field_inv_element]>>
       gs[]>>
       ,
   ...
   qabbrev_tac‘x =  dt ⊗ Inv (GF q) de’>>
   ‘de = (e1 ⊖ e2)’ by rw[] >>
   ‘de ≠ _0_’  by rw[GF_sub_not_eq_zero]>>
   ‘dt = (t1 ⊖ t2)’ by rw[] >>
